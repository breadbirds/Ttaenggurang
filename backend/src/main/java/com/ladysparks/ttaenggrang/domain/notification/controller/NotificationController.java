...
public class NotificationController implements NotificationApiSpecification {
    ...
    private final FirebaseCloudMessageWithDataService serviceWithData;

    /**
     * 📌 개별 메시지 전송 (기본 메시지)
     */
    @PostMapping("/send")
    public ResponseEntity<ApiResponse<NotificationDTO>> sendMessageTo(@RequestBody @Valid NotificationDTO notificationDTO) throws IOException {
        logger.info("📨 sendMessageTo: token={}, title={}, body={}",
                notificationDTO.getTargetToken(), notificationDTO.getTitle(), notificationDTO.getMessage());
        return ResponseEntity.ok(ApiResponse.success(service.sendMessageTo(notificationDTO)));
    }
    ...
    public ResponseEntity<ApiResponse<NotificationDTO>> sendDataMessageTo(@RequestBody @Valid NotificationDTO notificationDTO) throws IOException {
        logger.info("📨 sendDataMessageTo: token={}, title={}, body={}",
                notificationDTO.getTargetToken(), notificationDTO.getTitle(), notificationDTO.getMessage());
        return ResponseEntity.ok(ApiResponse.success(serviceWithData.sendDataMessageTo(notificationDTO)));
    }
