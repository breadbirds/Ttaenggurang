...
초등학교 교실을 작은 사회로 운영하며 학생들이 직접 급여·세금·투자를 경험하는
**교실 경제 활동**이 주목받는 추세.

전국 초등 교사 설문 결과, 경험 교사는 **수기 장부 관리·수동 정산**,
미경험 교사는 **정보 부족과 준비 부담**을 주요 어려움으로 지목.
기존 유사 앱(퍼플 등)은 서버 불안정·기능 부재로 현장 활용에 한계 존재.

정부의 **학생 1인 1태블릿 보급 정책**(2023년~)에 맞춰,
교사 부담은 줄이고 학생이 저축·투자·소비를 직접 체험할 수 있는
태블릿 기반 경제 교육 서비스 **땡그랑**을 기획.
...
- 📊 [초등 경제 시뮬레이션 활동 지원 앱 교사 설문조사](https://docs.google.com/spreadsheets/d/1xawRRV_nni4o17tQauIydjfObSsppd-zmwbBY5wpwHg/edit?gid=913541143)
- 📄 전라북도교육청, 「학생 교육용 스마트기기 보급 계획 안내」
- 📘 김미경, 『초등학교 경제 개념 학습을 위한 시뮬레이션 게임의 개발 및 적용』, 2003
...
> 비지도 학습으로 군집을 생성하고, 지도 학습으로 실시간 예측까지 연결한 2단계 ML 파이프라인

| 단계            | 내용                                                                                                 |
| --------------- | ---------------------------------------------------------------------------------------------------- |
| **비지도 학습** | 10만 건 가상 데이터 기반 K-Means / DBSCAN / GMM 3종 학습 → Silhouette Score 기준 최적 모델 자동 선택 |
| **군집 매핑**   | 저축률·소비 비율·투자 비율 기준으로 **소비형 / 저축형 / 투자형** 3가지 유형으로 자동 분류            |
| **지도 학습**   | SMOTE 오버샘플링 후 Random Forest 분류기 학습 → 테스트 정확도 **98.89%** 달성                        |
| **서비스 연동** | FastAPI `/predict-cluster` 엔드포인트 → Spring Boot 백엔드와 REST API 연동                           |

<img src="/docs/image/Ai활용.gif" width = "800">

### 2. OpenAI 프롬프팅을 활용한 경제 뉴스 생성

> 단순 API 호출이 아닌, 초등학생 눈높이에 맞는 프롬프트 설계와 응답 파싱까지 직접 구현

- 교사가 버튼 클릭 시 랜덤 주식 종목 선택 → **GPT-3.5-turbo** 호출
- 직접적인 주가 언급 대신 산업 동향·환경 변화를 간접 제시하는 공시 형태로 프롬프트 설계
- 응답을 **제목 / 내용 / 유형(호재·악재)** 으로 파싱 후 DB 저장 → **FCM 푸시 알림**으로 학생 즉시 전송
- 학생의 주간 금융 데이터 + 소비 유형을 GPT에 전달 → **개인화 주간 피드백** 2~3개 항목 생성
